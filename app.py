...
### Credit cost of every action that calls OpenAI
COSTS = {
    "cv_analysis": int(os.getenv("COST_CV_ANALYSIS", "1")),
    "ats_scan":    int(os.getenv("COST_ATS_SCAN", "1")),
    "job_cv":      int(os.getenv("COST_JOB_CV_UPLOAD", "1")),   
    "job_match":   int(os.getenv("COST_JOB_MATCH", "1")),       
    "job_detail":  int(os.getenv("COST_JOB_DETAIL", "1")),      
}
...
def allowed_file(filename, file_stream=None):
    valid_ext = os.path.splitext(filename)[1].lower() in ALLOWED_EXTENSIONS
    if not valid_ext:
        return False
    if file_stream:
...
def load_user(user_id):
    ...
    with conn.cursor() as cursor:
        cursor.execute("SELECT id, email, name, credits_remaining FROM users WHERE id = %s", (user_id,))
        row = cursor.fetchone()
    conn.close()
    if not row:
        return None
...
def get_or_create_user(google_id, email, name):
    conn = get_db()
    with conn.cursor() as cursor:
        cursor.execute("SELECT id, email, name, credits_remaining FROM users WHERE google_id = %s", (google_id,))
        row = cursor.fetchone()
        if row:
            conn.close()
            is_admin = row[1].lower() in ADMIN_EMAILS
...
def charge_credits(user, amount):
    if user.is_admin or amount <= 0: return True
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE users SET credits_remaining = credits_remaining - %s WHERE id = %s AND credits_remaining >= %s",
...
def refund_credits(user, amount):
    if user.is_admin or amount <= 0: return
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("UPDATE users SET credits_remaining = credits_remaining + %s WHERE id = %s", (amount, user.id))
    conn.commit()
    conn.close()

def credits_left(user):
    if user.is_admin: return "Unlimited"
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("SELECT credits_remaining FROM users WHERE id = %s", (user.id,))
        row = cur.fetchone()
    conn.close()
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"""
                List the 6-8 most important, specific skills required for a "{target_role}" role in {country} in 2026. Be specific to THIS role — do not default to generic data/tech skills unless the role is actually a data/tech role.
...
def fetch_dynamic_benchmark(target_role, country):
    conn = get_db()
    with conn.cursor() as cursor:
        cursor.execute("SELECT required_skills FROM global_benchmarks WHERE role_name ILIKE %s AND country = %s", (f'%{target_role}%', country))
        row = cursor.fetchone()
        if row:
            conn.close()
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"""
                Perform a deep-scan comparison of this CV against the provided Job Description (JD). Return JSON with: name, score (0-100), keywords (20-25 missing terms), rewrites (6-10 objects with 'original' and 'suggested' keys), weak_verbs (array).
                CV: {cv_text[:4000]}
                JD: {jd_text[:3000]}
                """}
            ],
...
def fetch_page_text(url):
    ...
    try:
        r = requests.get(url, timeout=8, allow_redirects=True, headers={
            ...
            "Accept-Language": "en"})
        final_url = r.url if is_safe_url(r.url) else ''
...
def get_cv_profile():
    ...
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("SELECT profile_json, cv_text FROM cv_profiles WHERE id = %s AND user_id = %s", (cv_id, current_user.id))
        row = cur.fetchone()
    conn.close()
...
def extract_job_intel(title, company, snippet, page_text, profile):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "Extract structured data from job postings. ALWAYS write every output value in English. Keep company names unchanged. Use ONLY the text given. Return ONLY JSON."},
            {"role": "user", "content": f"""
            ...
            *  company_website: URL if present in text, else null
            {match_spec}
            """}
        ]
    )
...
def fetch_jobs():
    ...
    CURRENCY = {'gb': '£', 'us': ' $', 'in': '₹', 'au': 'A$ ', 'ca': 'C $', 'ae': 'AED ', 'de': '€', 'fr': '€', 'es': '€', 'it': '€', 'nl': '€', 'at': '€', 'be': '€', 'ch': 'CHF ', 'pl': 'PLN ', 'sg': 'S$ ', 'nz': 'NZ $', 'za': 'R', 'br': 'R$ ', 'mx': 'MX$'}
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"From this CV return JSON: skills (array, max 25, only skills actually present), titles (array of job titles held), years_of_experience (number), suggested_role (string, best-fit target role).\n\nCV: {text[:4000]}"}
            ]
        )
        ...
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO cv_profiles (user_id, profile_json, cv_text) 
                VALUES (%s, %s, %s) 
                RETURNING id
            """, (current_user.id, json.dumps(profile), text[:8000]))
            session['cv_id'] = cur.fetchone()[0]
        conn.commit()
        conn.close()
...
def clear_cv():
    ...
    if cv_id and current_user.is_authenticated:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute("DELETE FROM cv_profiles WHERE id = %s AND user_id = %s", (cv_id, current_user.id))
        conn.commit()
        conn.close()
...
def job_detail():
    ...
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("SELECT data FROM job_details_cache WHERE url_hash = %s", (key,))
        row = cur.fetchone()
    conn.close()
    ...
    with conn.cursor() as cur:
        cur.execute("""
            INSERT INTO job_details_cache (url_hash, data) 
            VALUES (%s, %s)
...
def apply_list_score(intel, link):
    cv_id = session.get('cv_id')
    if not cv_id or not isinstance(intel.get('match'), dict): return intel
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("SELECT data FROM job_details_cache WHERE url_hash = %s", (score_cache_key(link, cv_id),))
        row = cur.fetchone()
    conn.close()
    if row: intel['match']['score'] = json.loads(row[0])['score']
    return intel
...
def match_jobs():
    profile = get_cv_profile()
    if not profile: return jsonify({"scores": {}})
        ...
        try:
            ...
            resp = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"""
                    CANDIDATE: skills={profile.get('skills')}, years_experience={profile.get('years_of_experience')}, titles={profile.get('titles')} 
                    CV EXCERPT: {profile['_text'][:1500]}
                    JOBS: {listing}
                    Score each job 0-100 using: skill overlap, role/seniority fit, years of experience, industry relevance. Be consistent and realistic; do not give everyone the same score. Return JSON: {{"scores": [{{"id": "<id>", "score": <int>}}]}}"""}
                ]
            )
